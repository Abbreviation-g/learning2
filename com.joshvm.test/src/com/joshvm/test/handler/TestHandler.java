...
public class TestHandler extends AbstractHandler {
	...
	private boolean isMenu2Enable = true;

	private void disableMenu2(ExecutionEvent event) {
		IWorkbenchSite workbenchSite = HandlerUtil.getActiveSite(event);
		ICommandService commandService = workbenchSite.getService(ICommandService.class);
		Command command2 = commandService.getCommand(TestHandler2.COMMAND_ID);
		ExpressionContext expressionContext = (ExpressionContext) event.getApplicationContext();
		isMenu2Enable = !isMenu2Enable;
		expressionContext.addVariable(TestHandler2.HANDLER2_ENABLE_VARIABLE, isMenu2Enable);
		command2.setEnabled(expressionContext);
		
		commandService.refreshElements(TestHandler2.COMMAND_ID, null);
		IHandler handler = command2.getHandler();
		if (handler instanceof AbstractHandler) {
			IEclipseContext appContext = ((Workbench)workbenchSite.getWorkbenchWindow().getWorkbench()).getApplication()
					.getContext();
			((HandlerServiceHandler) handler).fireHandlerChanged(new HandlerEvent(handler, true, false));
		}
	}
	...
	private void testEditorPart(ExecutionEvent event) {
		...
		if (editorPart instanceof ITextEditor) {
			...
			org.eclipse.jface.text.TextSelection selection = (TextSelection) textEditor.getSelectionProvider()
					.getSelection();
	...
	private void testNLS() {
		...
		ImageDescriptor imageDescriptor = Activator.getDefault().imageDescriptorFromPlugin(Activator.PLUGIN_ID,
				"$nl$/icons/main.png");
	...
	private void testAst(ExecutionEvent event) {
		...
		IProgressService progressService = HandlerUtil.getActiveWorkbenchWindow(event).getWorkbench()
				.getProgressService();
	...
	private void testDevice() {
		try {
			for (IDevice device : MTJCore.getDeviceRegistry().getAllDevices()) {
				System.out
						.println("TestHandler.testDevice()" + device.getIdentifier() + " -> " + device.getClasspath());
			}
